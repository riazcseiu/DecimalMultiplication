...
static void      decFiniteMultiply(bcdnum *, uByte *, const decFloat *,
                              const decFloat *);
static void      MdecFiniteMultiply(bcdnum *,  const decFloat *,
	const decFloat *);
...
#define MULOPLEN  DECPMAX9         //2 for 64 bit //operand length ('digits' base 10**9)
#define MULACCLEN (MULOPLEN*2)    // 4 for 64 bit// accumulator length (ditto)
#define LEADZEROS (MULACCLEN*9 - DECPMAX*2) // 36 - 32 = 4 for 64 bit //leading zeros always

// Assertions: exponent not too large and MULACCLEN is a multiple of 4
#if DECEMAXD>9
  #error Exponent may overflow when doubled for Multiply
#endif
#if MULACCLEN!=(MULACCLEN/4)*4 // must be setiffy 
...
static void decFiniteMultiply(bcdnum *num, uByte *bcdacc,
  ...
  uInt   bufl[MULOPLEN];           //2 for 64 bit left //uInt = 32 bit// coefficient (base-billion)
  uInt   bufr[MULOPLEN];           //2 for 64 bit right coefficient (base-billion)
  uInt   *ui, *uj;                 // work
  uByte  *ub;                      // ..uByte =Uint8_t
  uInt   uiwork;                   // for macros //uInt = 32 bit
  
  #if DECUSE64
  uLong  accl[MULACCLEN];          //MULACCLEN = 4 fro 64 bit//uLong = unit64_t// lazy accumulator (base-billion+)
 ...
 //printf("Base10**9: OpLen=%d MulAcclen=%d , ledzero%d\n", MULOPLEN, MULACCLEN , LEADZEROS);
...
#if DECUSE64
	//r_pl is 64 bit and pa is 32 bit int pointer
  // zero the accumulator
  #if MULACCLEN==4 //4 for 64 bit
  ...
  //r_*ui , *uj, *pa are 32 bit and accl , *pl are 64 bit size bufl , bufr is array of 2 element size 32 bit
  //R_ Base10E9: OpLen=2 MulAcclen=4 , ledzero4
  ...
  #define MULMAGIC 2305843009U          // 2**61/10**9 (unsigned) [both cases]
  #if DOUBLE
    #define MULSHIFTA 29
    #define MULSHIFTB 32
  #elif QUAD
  ...
  printf("\n");
  #endif 
  //r_value of accumulator for 64 bit calculation 
  //r_*ui , *uj, *pa , acc[4]are 32 bit and accl[4] , *pl are 64 bit size bufl[2] , bufr[2] is array of 2 element size 32 bit
  //r_ one  BCD digit ineach byte (BCD8 encoding) 4 = {0000 0100}
  //r_ Base =10E9: OpLen=2 MulAcclen=4 , ledzero4
  for (pl=accl, pa=acc; pl<accl+MULACCLEN; pl++, pa++) { // each column position
    uInt lo, hop;                       // work r_ 32 bit
    uInt est;                           // cannot exceed 4E+9
    if (*pl>=MULTBASE) {                //r_MULTBASE=1E9
      // *pl holds a binary number which needs to be split
      hop=(uInt)(*pl>>MULSHIFTA); //right-shift 29 bit
  ...
  #endif
  //-----------------------------------------
  //r_ poiter to array assignment Example
  // int ary[5]={1,2,3,4,5};
  //int *d;
 // d = ary;
 // *d =1; *(d + 1) = 2 ; 
 //-----------------------------------------------------
  ...
  //r_ Base =10E9: OpLen=2 MulAcclen=4 , ledzero=4
  pa=acc+MULACCLEN-1;
  if (*pa!=0) num->msd=bcdacc+LEADZEROS;// bcdaxdrop known lead zeros
  ...
  for (ub=bcdacc;; pa--, ub+=9) {        //uByte  *ub;//
  ...
  ///new modifid finite mutiply function fro multiplicationi 

  static void MdecFiniteMultiply(bcdnum *num, 
	  const decFloat *dfl, const decFloat *dfr) {
	  uInt   bufl[MULOPLEN];           //2 for 64 bit left  coefficient (base-billion)
	  uInt   bufr[2];           //2 for 64 bit right coefficient (base-billion)
	  uInt   *ui, *uj;                 // work
	  uByte  *ub;                      // ..
	  uInt   uiwork;                   // for macros

#if DECUSE64
	  uLong  accl[2];          // lazy accumulator (base-billion+)
	  uLong  *pl;                      // work -> lazy accumulator
	  //uInt   acc[MULACCLEN];           // coefficent in base-billion ..
	  uLong acc[2];// it should be equal to MULACCLE but here 2
#else
	  uInt   acc[MULACCLEN * 2];         // accumulator in base-billion ..
#endif
	  uInt   *pa;
	  ...
	  num->sign = (DFWORD(dfl, 0) ^ DFWORD(dfr, 0)) & DECFLOAT_Sign;
	  num->exponent = GETEXPUN(dfl) + GETEXPUN(dfr); // [see assertion above]
	   //Base10E9: OpLen=2 MulAcclen=4 , ledzero4
	  //printf("Base10**9: OpLen=%d MulAcclen=%d , ledzero%d\n", MULOPLEN, MULACCLEN, LEADZEROS);
													 /* Extract the coefficients and prepare the accumulator */
													 // the coefficients of the operands are decoded into base-billion
													 // numbers in uInt arrays (bufl and bufr, LSD at offset 0) of the
													 // appropriate size.
	  //Macro for GETCOEFF getting coefficet in bcd format 
	  //uByte *bcdar[DECPMAX];
	 //decFloatGetCoefficient(dfl, bcdar);
	 // GETCOEFF(df, bcdar);
	// step1
...
//---------------------------------------------------------------
	  ...
	  //bellow code extract coefficeint in base bilion form decfloat format 
	      uInt sourhi, sourlo;                             \
		  sourlo = DFWORD(dfl, 1);                          \
		  ...
		  sourhi = DFWORD(dfl, 0);                            \
		  (bufl)[1] = DPD2BIN0[((sourhi << 2) | (sourlo >> 30)) & 0x3ff] \
...
//--------------------------------------------------------------------------------------
//need to comment out for addition option ------------------------------------------

	  // start the 64-bit/32-bit differing paths...
		 //for (int precomputei = 2; precomputei < 6; precomputei+=3) // it calculate 2X and 5X with conversion
		...
		//4 item for64 bit
	 // Long  accPrecompute[9];
	  
	  //accPrecompute[0]= bufl;
	  //accPrecompute[1] = *bufl*2;
	 // accPrecompute[2] = *bufl * 3;
	 // accPrecompute[3] = *bufl * 4;
	 // accPrecompute[4] = *bufl * 5;
	 // accPrecompute[5] = *bufl * 6;
	 // accPrecompute[6] = *bufl * 7;
	  //accPrecompute[7] = *bufl * 8;
	 // accPrecompute[8] = *bufl * 9; 
	  ...
	  accl[0] = 0; accl[1] = 0;// accl[2] = 0; accl[3] = 0;
		...
		/* Effect the multiplication 
	       ...
	       ui = bufr;  
		   pl = accl+(ui - bufr);// where to add the lhs
		  for (uj = bufl; uj<bufl + 2; uj++, pl++) { // over each item in lhs here 2 is the length for 64 bit 
															// if (*uj==0) continue;            // product cannot affect result
			  *pl += ((uLong)*ui)*(*uj);
			  //printf(" %ld\n", (LI)*pl);
		  } // uj
	  // ui
		//4 item for64 bit*/
	       pl = accl;
		  *pl = bufl[0] * precomputei;
		    
		  *pl += bufl[1]*precomputei;
...
#define MULSHIFTB 32
...
#endif 
	  //r_value of accumulator for 64 bit calculation 
	  //r_*ui , *uj, *pa , acc[2]are 32 bit and accl[4] , *pl are 64 bit size bufl[2] , bufr[2] is array of 2 element size 32 bit
	  //Base10E9: OpLen=2 MulAcclen=4 , ledzero=4

	  for (pl = accl, pa = acc; pl<accl + 2; pl++, pa++) { // each column position
		  uInt lo, hop;                       // work
		  uInt est;                           // cannot exceed 4E+9
		  if (*pl >= MULTBASE) {
			  // *pl holds a binary number which needs to be split
	  ...
	  // Now convert to BCD for rounding and cleanup, starting from the
	  // most significant end
	  uByte  bcdacc[DECPMAX9 * 18 + 1]; // 16*18+1 = 289 eleement with sizze unit8_t // #define uByte  uint8_t //uint8_t =unsigned char - 
	  //pa = acc + MULACCLEN - 1;
	  pa = acc + 2 - 1;
	  if (*pa != 0) num->msd = bcdacc + LEADZEROS;// bcdaxdrop known lead zeros
	  else {                               // >=1 word of leading zeros
...
decFloat * MdecFloatMultiply(decFloat *result,
	const decFloat *dfl, const decFloat *dfr,
	decContext *set) {
	bcdnum num;                      // for final conversion
	uByte  bcdacc[DECPMAX9 * 18 + 1];    // for coefficent in BCD
	...
	/* Here when operands is finite */
	MdecFiniteMultiply(&num,  dfl, dfr);
	return dfr;
	//return decFinalize(result, &num, set); // round, check, and lay out
} // decFloatMultiply
